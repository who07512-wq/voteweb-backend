...
router.post('/', requireAdmin, electionController.create.bind(electionController));

// PATCH /api/v1/admin/elections/:id - Update election (admin only)
router.patch('/:id', requireAdmin, electionController.update.bind(electionController));

// PATCH /api/v1/admin/elections/:id/status - Update election status (admin only)
router.patch('/:id/status', requireAdmin, electionController.updateStatus.bind(electionController));
...
router.post('/:id/publish', requireAdmin, electionController.publishResults.bind(electionController));
